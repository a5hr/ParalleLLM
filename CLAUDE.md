...
`data/models.json` が単一ソース。`scripts/update-models.ts` が全プロバイダー (OpenAI, Anthropic, Google, Groq, OpenRouter) のAPIからモデル一覧を取得し、`data/model-overrides.json` の `include` (採用モデル一覧) と `overrides` (APIで取得できない料金・機能等の補完) とマージして生成。GitHub Actionsで毎日自動更新（JST 08:00）。差分があればレビューレポート付きPRを自動作成。
